...
- 后台加载：上传 MP4 后会在后台解析素材，避免控制面板长时间卡住。
- 自动恢复：动作素材路径、大小、背景色和容差会保存到本地 `pet_settings.json`，下次启动自动加载。
- 透明设置：选择背景色并调整容差。
- 大小：使用 50% 到 250% 滑块调整宠物显示大小。
- 上传素材会按素材中宠物的实际裁剪尺寸显示，大小滑块表示在素材尺寸基础上的缩放比例。
...
- 如果控制面板仍是默认绿色，但素材角落明显不是绿色，程序会自动识别角落背景色，例如黑底素材会自动按黑色处理。
- 程序会对边缘做柔化和高饱和黄/红碎片清理，减少宠物移动时的背景残影。
- 如果透明效果不理想，调低或调高容差，或换成纯色背景更干净的视频。

本地配置说明：
- `pet_settings.json` 是运行时自动生成的本机配置文件，里面保存你的素材路径。
- 这个文件已经加入 `.gitignore`，不会被提交到 GitHub。
